...
:build_script
bash -lc "cd $APPVEYOR_BUILD_FOLDER; meson build -Ddri-drivers= -Dgallium-drivers=swrast -Dvulkan-drivers= -Dplatforms=x11,surfaceless -Dglx=dri -Degl=false --wrap-mode=nofallback && ninja -C build && ccache -s"
goto :eof
