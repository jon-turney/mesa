...
:build_script
bash -lc "cd $APPVEYOR_BUILD_FOLDER; meson build -Ddri-drivers= -Dgallium-drivers=swrast -Dplatforms=x11,surfaceless -Dglx=xlib -Dvulkan-drivers= --wrap-mode=nofallback && ninja -C build && ccache -s"
goto :eof
